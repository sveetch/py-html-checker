...
def mock_validator_execute_validator_for_base_exception(*args, **kwargs):
    ...
    value of ``EXCEPTION_PATH_TRIGGER`` (using an http url instead of a path
...
def test_page_safe_exception(monkeypatch, caplog, settings):
    ...
    expected = [
        ...
        ("py-html-checker", logging.INFO, EXCEPTION_PATH_TRIGGER),
        ("py-html-checker", logging.ERROR, "This is a dummy exception."),
        (
            "py-html-checker",
            logging.INFO,
    ...
    with runner.isolated_filesystem():
        ...
        expected = [
            ...
            (
                "py-html-checker",
                logging.INFO,
                "Created file: {}".format(expected_document)
            ),
        ]
